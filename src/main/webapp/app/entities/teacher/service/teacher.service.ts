...
type RestOf<T extends ITeacher | NewTeacher> = Omit<T, 'hireDate'> & {
...
export class TeacherService {
  ...
  protected convertDateFromClient<T extends ITeacher | NewTeacher | PartialUpdateTeacher>(teacher: T): RestOf<T> {
    return {
      ...teacher,
      hireDate: teacher.hireDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTeacher: RestTeacher): ITeacher {
    return {
      ...restTeacher,
      hireDate: restTeacher.hireDate ? dayjs(restTeacher.hireDate) : undefined,
    };
  }
