...
type RestOf<T extends IStudent | NewStudent> = Omit<T, 'enrollmentDate' | 'graduationDate'> & {
...
export class StudentService {
  ...
  protected convertDateFromClient<T extends IStudent | NewStudent | PartialUpdateStudent>(student: T): RestOf<T> {
    return {
      ...student,
      enrollmentDate: student.enrollmentDate?.format(DATE_FORMAT) ?? null,
      graduationDate: student.graduationDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restStudent: RestStudent): IStudent {
    return {
      ...
      graduationDate: restStudent.graduationDate ? dayjs(restStudent.graduationDate) : undefined,
    };
  }
